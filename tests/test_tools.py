...
class TestLognormpdf:
    def test_scalar_output(self):
        ...
        expected = stats.multivariate_normal.logpdf(
            x.reshape(-1), mean=mean, cov=cast(Any, cov)
        )
        ...
        expected = np.array(
            [
                stats.multivariate_normal.logpdf(
                    x[:, 0], mean=mean, cov=cast(Any, cov)
                ),
                stats.multivariate_normal.logpdf(
                    x[:, 1], mean=mean, cov=cast(Any, cov)
                ),
            ]
        )
    ...
    def test_column_vector_mean(self):
        ...
        result = lognormpdf(x, mean, cov)
        expected = stats.multivariate_normal.logpdf(
            x, mean=mean.flatten(), cov=cast(Any, cov)
        )
...
class TestLogBanana:
    def test_banana_transform(self):
        ...
        # Compute the transformed point manually (y0 = x0 - shift, y1 = x1 + y0^2)
        y0 = x[0]
        y1 = x[1] + y0**2
        y = np.array([y0, y1])
        ...
        expected = stats.multivariate_normal.logpdf(
            y.reshape(-1), mean=mu, cov=cast(Any, sigma)
        )
    ...
    def test_multiple_points(self):
        ...
        # Manually compute transformed points (y0 = x0, y1 = x1 + y0^2)
        y0 = x[0, :]
        y1 = x[1, :] + y0**2
        ...
        expected = np.array(
            [
                stats.multivariate_normal.logpdf(
                    [y0[0], y1[0]], mean=mu, cov=cast(Any, sigma)
                ),
                stats.multivariate_normal.logpdf(
                    [y0[1], y1[1]], mean=mu, cov=cast(Any, sigma)
                ),
            ]
        )
