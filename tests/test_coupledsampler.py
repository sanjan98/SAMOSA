"""
Unit tests for the coupledMCMCsampler class.
"""
...
class MockProposal(ProposalProtocol):
    ...
    def __init__(self, sigma=None):
        self.sigma = np.eye(2) if sigma is None else sigma
...
class MockKernel(KernelProtocol):
    ...
    def propose(self, proposal_coarse, proposal_fine, current_coarse_state, current_fine_state):
        """Return mock proposed states."""
        self.propose_called = True
        # Create new states with slightly modified positions
    ...
    def acceptance_ratio(self, proposal_coarse, current_coarse, proposed_coarse, 
                         proposal_fine, current_fine, proposed_fine):
        """Return mock acceptance ratios."""
        self.acceptance_ratio_called = True
        # Return fixed acceptance ratios for testing
        return 0.7, 0.8
    
    def adapt(self, proposal_coarse, current_coarse, proposal_fine, current_fine):
        """Record that adapt was called."""
        self.adapt_called = True
        if hasattr(proposal_coarse, 'adapt'):
            proposal_coarse.adapt(current_coarse)
        if hasattr(proposal_fine, 'adapt'):
            proposal_fine.adapt(current_fine)
...
@pytest.fixture
def coupled_sampler(coarse_model, fine_model, kernel, proposal_coarse, proposal_fine, 
                   initial_position_coarse, initial_position_fine):
...
# Tests
# --------------------------------------------------
def test_coupled_sampler_init(coupled_sampler, coarse_model, fine_model, kernel, 
                             proposal_coarse, proposal_fine, initial_position_coarse, 
                             initial_position_fine):
    """Test initialization of coupledMCMCsampler."""
    ...
    assert np.array_equal(coupled_sampler.initial_state_coarse.position, initial_position_coarse)
    assert np.array_equal(coupled_sampler.initial_state_fine.position, initial_position_fine)
...
def test_coupled_sampler_dimension_mismatch(coarse_model, fine_model, kernel, 
                                          proposal_coarse, proposal_fine):
...
def test_coupled_sampler_load_samples(coupled_sampler, output_dir, monkeypatch):
    ...
    # Load the samples
    samples_coarse, samples_fine = coupledMCMCsampler.load_samples(output_dir)
...
def test_coupled_sampler_different_iterations(coarse_model, fine_model, kernel, 
                                            proposal_coarse, proposal_fine,
                                            initial_position_coarse, initial_position_fine,
...
def test_coupled_sampler_deep_copy(coupled_sampler, output_dir, monkeypatch):
    ...
    # Verify that each sample has its own metadata dictionary (not shared references)
    for i in range(1, len(samples_coarse)):
        # Modify metadata in one sample
        samples_coarse[i-1].metadata['test_key'] = 'test_value'
        
        # Check that it didn't affect the next sample
