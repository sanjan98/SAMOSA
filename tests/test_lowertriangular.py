...
def banana_samples():
    ...
    for i in range(num_points):
        ...
        samples.append(ChainState(position=position, log_posterior=None))
...
def triangular_map(banana_samples):
    ...
    tmap = LowerTriangularMap(dim=2, total_order=2)
    tmap.adapt(samples, force_adapt=True)
    return tmap
...
def test_forward_transformation_normalization(triangular_map, banana_samples):
    ...
    assert np.allclose(r_mean, 0, atol=1e-2), (
        f"Transformed mean is not close to 0: {r_mean.flatten()}"
    )
    # Assert that the standard deviation of the transformed points is close to 1
    assert np.allclose(r_std, 1, atol=1e-2), (
        f"Transformed std is not close to 1: {r_std.flatten()}"
    )
