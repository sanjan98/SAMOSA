...
class GaussianRandomWalk(ProposalBase):
    ...
    def __init__(self, mu: np.ndarray, cov: np.ndarray) -> None:
        ...
        if mu.shape[0] != dim:
            raise ValueError(
                f"Mean dimension {mu.shape[0]} does not match covariance dimension {dim}"
            )
    ...
    def proposal_logpdf(
        ...
        """
        logq_forward = lognormpdf(
            proposed_state.position, current_state.position + self.mu, self.cov
        )
        logq_reverse = lognormpdf(
            current_state.position, proposed_state.position + self.mu, self.cov
        )
...
class IndependentProposal(ProposalBase):
    ...
    def __init__(self, mu: np.ndarray, cov: np.ndarray) -> None:
        ...
        if mu.shape[0] != cov.shape[0]:
            raise ValueError(
                f"Mean dimension {mu.shape[0]} does not match covariance dimension {cov.shape[0]}"
            )
...
class PreCrankedNicholson(ProposalBase):
    ...
    def proposal_logpdf(
        ...
        mean_forward = (
            np.sqrt(1 - self.beta**2) * (current_state.position - self.mu) + self.mu
        )

        # Reverse: current = sqrt(1-β²)(proposed-μ) + μ + β·z
        mean_reverse = (
            np.sqrt(1 - self.beta**2) * (proposed_state.position - self.mu) + self.mu
        )
    ...
    def adapt(self, state: ChainState) -> None:
        ...
        self.long_average = self.long_alpha * self.long_average + is_accepted * (
            1 - self.long_alpha
        )
        self.short_average = self.short_alpha * self.short_average + is_accepted * (
            1 - self.short_alpha
        )
