"""
Class file for the coupled Independent kernel for coupled chain
"""
...
class CoupledIndependentKernel(KernelProtocol):
    """
    Independent coupled kernel for Multi-Level MCMC sampling.
    Maintains two chains (high-fidelity and low-fidelity) and proposes coupled moves.
    Proposes a common point using an independent sampler.
    """

    def __init__(self, coarse_model: ModelProtocol, fine_model: ModelProtocol, sampler: Optional[ProposalProtocol] = None):
        """
    ...
    def propose(self, proposal_coarse: ProposalProtocol, proposal_fine: ProposalProtocol, current_coarse_state: ChainState, current_fine_state: ChainState) -> Tuple[ChainState, ChainState, ChainState, ChainState]:
        ...
        if self.sampler is None:
            ...
            self.sampler = IndependentProposal(mu = common_mean, sigma = common_cov)

        eta = self.sampler.sample(current_fine_state)
    ...
    def adapt(self, proposal_coarse: ProposalProtocol, proposed_coarse: ChainState, proposal_fine: ProposalProtocol, proposed_fine: ChainState) -> None:
        ...
        Returns:
            None
        """
        pass # No adaptation for independent kernel