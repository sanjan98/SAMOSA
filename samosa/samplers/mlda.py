...
class MLDASampler:
    ...
    """
    def __init__(self, models: list[ModelProtocol], kernel: KernelProtocol, proposal: ProposalProtocol, initial_position: np.ndarray, n_iterations: int, subchain_lengths: list[int], use_aem: bool = False, print_iteration: Optional[int] = None):
        """
        Parameters:
    ...
    def run(self, output_dir: str):
        ...
        initial_state = ChainState(position=self.initial_position, **self.models[-1](self.initial_position), metadata={
            'acceptance_probability': 0.0,
            'iteration': 1
            })

        _ = self._recursive_mlda(self.models, self.kernel, self.proposal, self.subchain_lengths, initial_state, self.L, self.n_iterations, samples)

        self._save_samples_levelwise(samples, self.subchain_lengths, output_dir)

    def _recursive_mlda(self, models, kernel, proposal, subchain_lengths, initial_state, L, N, samples):
        ...
        for j in range(1, N+1):
            ...
            if L == 1:
                sampler = MCMCsampler(kernel, proposal, current_state.position, subchain_lengths[0], print_iteration=self.print_iteration) 
                ...
                subchain = self._recursive_mlda(models[:-1], kernel, proposal, subchain_lengths[:-1], current_state, L-1, subchain_lengths[-1], samples)

            proposed_state = ChainState(position=subchain[-1].position, **models[-1](subchain[-1].position), metadata=current_state.metadata.copy())

            current_coarse_logpost = models[-2](current_state.position)['log_posterior']
            proposed_coarse_logpost = subchain[-1].log_posterior
            
            ar = self._mlda_ar(current_state, proposed_state, current_coarse_logpost, proposed_coarse_logpost)
    ...
    def _mlda_ar(self, current_state: ChainState, proposed_state: ChainState, current_coarse: np.ndarray, proposed_coarse: np.ndarray) -> float:
        """
...
if __name__ == "__main__":
    ...
    mlda_sampler = MLDASampler(models=models_list, kernel=kernel, proposal=proposal, initial_position=initial_position, n_iterations=n_iterations, subchain_lengths=subchain_lengths, use_aem=False, print_iteration=1000)
    mlda_sampler.run(output_dir='mlda_test_output')

    mlmc_calc = MLMCCalculator(output_dir='mlda_test_output', num_levels=L, print_progress=True)
